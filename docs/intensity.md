...
Deep targets are ranked by direct disease relevance, sample-material tissue/source evidence versus background mentions, patient/control context, and supported criteria. The preselected SOFT list is twice the deep-review cap so rule-gated studies can be replaced. If rule gating still leaves model slots empty, the next-ranked studies are appended, up to four times the cap in total downloads. Downloading stops once the number of studies sent to the model reaches that cap. Similar titles are deferred only among near scores so diversity cannot replace a much stronger hit with an off-tissue study. The selected IDs are checkpointed before fetching so a resume does not change the target list. Ranking is a heuristic, not eligibility evidence or a probability.
