...
4. 强度选 **Low**（发现候选，默认不做 SOFT 深核），再点 **开始核验**。
   Low 大约 80 个唯一 GSE、0 次 SOFT。只看 NCBI 摘要和规则初筛，几乎不会出现「推荐」。
   课题值得继续时，再选 **Medium** 或更高。四档是预算，不是准确率等级。不要一上来就选 Ultra。
5. 看运行区：阶段、唯一 GSE、token。觉得够了就 **暂停** 或 **取消**（已发出的模型请求费用退不回）。
6. 三个页签：**推荐 / 待核实 / 排除**。点一行看详情、证据和 GSM。
7. 结束后 **导出 Excel**。初筛后推荐表空着、待核实很多，是正常的；Medium 及以上才会做 SOFT 和模型核验。
